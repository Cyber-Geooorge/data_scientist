...
def menu(journa_list)
    ...
    puts " 4  Handle qui commence avec une majuscule."
    ...
    n = gets.chomp.to_i
        while n >= 1 && n <=8
            if n == 1 
                puts "Il y a #{journa_list.count} journalistes dans ce tableau."

            elsif n == 2 
                puts "Le handle le plus court est #{journa_list.min_by(&:length)}"

            elsif n == 3
            without = remo_x(journa_list)
            nbr = chara_number(without)
            puts "Il y a #{nbr.count(5)} handles contenants 5 caractères." 

            elsif n == 4
            without = remo_x(journa_list)
            puts "Il y a #{without.grep(/^[A-Z]/).count} handles qui debutent avec une majuscule."

            elsif n == 5 
            puts journa_list.sort

            elsif n == 6
            puts journa_list.sort_by(&:length)
